...
class MainWindow(QWidget):
    ...
    def set_ui(self):
        ...
        self.btn_photo = QPushButton('录入人脸数据')
    ...
    def face_thread(self):  # 这里多线程是解决，同时识别不同人脸的关键
        ...
        for k, face in enumerate(self.dets):  # 遍历视频中所有人脸 ，
            ...
            if float(cd_sorted[0][1])<0.4:
                Name,Number,Class,Time=cd_sorted[0][0].split('_')[0],cd_sorted[0][0].split('_')[1],cd_sorted[0][0].split('_')[2],time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))

                # cv2.putText(self.show_3, Number, (x2, x1), cv2.FONT_HERSHEY_SIMPLEX, 1.0,
                #         (0, 255, 255), 2)
                # QMessageBox.information(self,u"成功",u"打卡成功！\n姓名：%s\n学号：%s\n班级：%s\n"%(Name,Number,Class),QMessageBox.Yes)
            # 各参数依次是：照片/添加的文字/左上角坐标/字体/字体大小/颜色/字体粗细

                
                conn,cursor=connect_sqlite()
                # create_table()
                # print('$$$insert into attend_log (name,number,class,time) values ("%s","%s","%s","%s")'%(Name,Number,Class,Time))
                cursor.execute('insert into attend_log (name,number,class,time) values ("%s","%s","%s","%s")'%(Name,Number,Class,Time))
                disconnect_sqlite(conn,cursor)
# insert into attend_log (name,number,class,time) values ("YF","F15315","F1R13","2019-04-29 22:03:35")
                print(u"打卡成功！\n姓名：%s\n学号：%s\n班级：%s\n"%(Name,Number,Class))
                time.sleep(10)
                ...
                cv2.putText(self.show_3, 'unknown', (x2, x1), cv2.FONT_HERSHEY_SIMPLEX, 1.0,
                        (0, 255, 255), 2)
            print("\n The maxinium possible person is: ", cd_sorted[0][0])
            detect_image = QImage(self.show_3.data, self.show_3.shape[1], self.show_3.shape[0],
                                  QImage.Format_RGB888)
            self.label_show_camera.setPixmap(QPixmap.fromImage(detect_image))
    ...
    def photo_face(self):
        ...
        if ok and (len(name)>5):
            photo_save_path = os.path.join(os.path.dirname(os.path.abspath('__file__')),
                                       'candidate-faces/')
            self.showImage.save(photo_save_path + name + ".jpg")
    ...
    def dlib_para_init(self):
        ...
        self.candidate = [o.split('.')[0] for o in os.listdir(faces_folder_path)]
